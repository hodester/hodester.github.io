...
//Source: http://www.satellite-calculations.com/TLETracker/SatTracker.htm - Jens T. Satre
...
function do_autoupdate() {
    ...
    if (autoupdate == "True") {
        ...
        if (Tracker.LongitudeDriftrate.value * 1 > 0)
            Tracker.LongitudeDriftrateDirection.value = "East"
        else
            Tracker.LongitudeDriftrateDirection.value = "West";
...
function CalculateCoordinate() {
    ...
    if (Tracker.LongitudeDirection.value == "West")
        EarthSiteLongitude = -SiteLon
    else
        EarthSiteLongitude = SiteLon;

    if (Tracker.LatitudeDirection.value == "South")
        EarthSiteLatitude = -SiteLat
    else
        EarthSiteLatitude = SiteLat;
...
function getCurrentTime() {
    ...
    if (Day < 10)
        ...
        ReturnValue[2] = Day;
    if (Hour < 10)
        ReturnValue[3] = ("0" + Hour)
    else
        ReturnValue[3] = Hour;
    if (Minute < 10)
        ReturnValue[4] = ("0" + Minute)
    else
        ReturnValue[4] = Minute;
    if (Second < 10)
        ReturnValue[5] = ("0" + Second)
    else
...
function get_time_in_epoch_format(selected_time, timefromepoch) {
    ...
    if ((theDay < 10) && (theDay < 100))
        DayStr = "00" + formatvalue(theDay, 4)
    else if ((theDay > 10) && (theDay < 100))
        DayStr = DayStr = "0" + formatvalue(theDay, 4)
...
function calculatedriftrate(Day, Month, Year, Hour, Minute, Second, Satellite_rev_solar_day, Satellite_rev_sidereal_day, first_derative_mean_motion, Eccentricity, Epoch, Meananomaly, Argument_of_perigee, RA_AN, Inclination, NumberOfOrbitsAtEpoch) {
    ...
    if (Rev(E) > 1 * OutputArray[18])
        ...
        OutputArray[38] = Rev(1 * OutputArray[18] - Rev(E))
    ...
    if (Longitude > 180)
        LonString = formatvalue(360 - Longitude, 10) + " West"
    else
        LonString = formatvalue(Longitude, 10) + " East";
    ...
    if (Latitude < 0)
        LatString = formatvalue(-Latitude, 10) + " South"
    else
        LatString = formatvalue(Latitude, 10) + " North";

    if (geodeticLat > 0)
        OutputArray[32] = formatvalue(Math.abs(geodeticLat), 10) + " North"
    else
        OutputArray[32] = formatvalue(Math.abs(geodeticLat), 10) + " South";
